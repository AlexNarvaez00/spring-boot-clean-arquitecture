...
public class TypeSupportAreaGetAllTest {
    ...
    void runTest() {
        ...
        Page<TypeSupportArea> type_support_areas = new PageImpl<
            TypeSupportArea
        >(new ArrayList<TypeSupportArea>(), pageReq, 0);

        Mockito.when(repo.findAll(pageReq)).thenReturn(type_support_areas);
        Page<TypeSupportAreaPrimitives> all = TypeSupportAreaGetAll.run(
            pageReq,
            repo
        );
