...
public class StudentControllerTest extends RestClientTest<StudentRepository> {
    ...
    void testDestroy() throws Exception {
        ...
        Mockito.when(
            this.getRepo().findStudentById(student.getId())
        ).thenReturn(student);
    ...
    void testStore() throws Exception {
        ...
        request.put("control_number", student.getControlNumber().getValue());
        request.put("profile_id", student.getProfile().getId().toString());
    ...
    void testUpdate() throws Exception {
        ...
        request.put(
            "control_number",
            studentUpdate.getControlNumber().getValue()
        );
        request.put(
            "profile_id",
            studentUpdate.getProfile().getId().toString()
        );

        Mockito.when(
            this.getRepo().findStudentById(student.getId())
        ).thenReturn(studentUpdate);
